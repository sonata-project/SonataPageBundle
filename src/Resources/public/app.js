...
PageComposer.prototype = {
  init() {
    ...
    this.bindOrphansHandlers(); // attach event listeners
  ...
  getRouteUrl(id, parameters) {
    if (!this.routes[id]) {
      throw new Error("Route \"".concat(id, "\" does not exist"));
    }

    let url = this.routes[id]; // eslint-disable-next-line no-restricted-syntax, guard-for-in
  ...
  handleBlockCreated(event) {
    const self = this;
    jQuery.ajax({
      url: this.getRouteUrl('block_preview', {
      ...
      success(resp) {
        ...
        self.controlChildBlock($content); // refresh parent block child count
  ...
  handleBlockParentSwitched(event) {
    const $previousParentPreview = jQuery(".block-preview-".concat(event.previousParentId));
    ...
    const $newParentPreview = jQuery(".block-preview-".concat(event.newParentId));
  ...
  getContainerChildCountFromList(containerId) {
    const $blockView = this.$dynamicArea.find(".block-view-".concat(containerId));
  ...
  updateChildCount(blockId, count) {
    const $previewCount = jQuery(".block-preview-".concat(blockId));
    const $viewCount = jQuery(".block-view-".concat(blockId));
  ...
  handleBlockCreateFormLoaded(event) {
    ...
    if (event.container) {
      ...
      $childBlock = jQuery(['<li class="page-composer__container__child">', '<a class="page-composer__container__child__edit">', '<h4 class="page-composer__container__child__name">', '<input type="text" class="page-composer__container__child__name__input">', '</h4>', '</a>', '<div class="page-composer__container__child__right">', "<span class=\"badge\">".concat(event.blockTypeLabel, "</span>"), '</div>', '<div class="page-composer__container__child__content">', '</div>', '</li>'].join(''));
      $childContent = $childBlock.find('.page-composer__container__child__content');
    ...
    $container.show(); // scan form elements to find name/parent/position,
    ...
    $formControls.each(function formControl() {
      ...
      if (self.isFormControlTypeByName(formControlName, 'name')) {
        ...
        $childName.find('.page-composer__container__child__name__input').on('propertychange keyup input paste', function onChange() {
    ...
    $formActions.each(function formActionItem() {
      ...
      const $cancelButton = jQuery("<span class=\"btn btn-warning\">".concat(self.translate('cancel'), "</span>"));
    ...
    $form.on('submit', e => {
      ...
      jQuery.ajax({
        url: "".concat(formAction, "&").concat(jQuery.param({
          composer: 1
        })),
        data: $form.serialize(),
        type: formMethod,
  ...
  handleBlockEditFormLoaded(event) {
    ...
    const blockType = event.$block.find('.page-composer__container__child__edit small').text().trim();
    let $nameFormControl;
    let $positionFormControl;
    $form.find('input').each(function input() {
      ...
      if (self.isFormControlTypeByName(formControlName, 'name')) {
        $nameFormControl = $formControl;
        $title.html("<input type=\"text\" class=\"page-composer__container__child__name__input\" value=\"".concat($title.text().trim(), "\">"));
  ...
  controlChildBlock($childBlock) {
    ...
    $edit.click(e => {
      e.preventDefault(); // edit form already loaded, just toggle
  ...
  confirmRemoveContainer($childBlock) {
    ...
    if ($removeDialog.length === 0) {
      $removeDialog = jQuery(['<div class="modal fade page-composer__container__child__remove__dialog" tabindex="-1" role="dialog">', '<div class="modal-dialog" role="document">', '<div class="modal-content">', '<div class="modal-header">', '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>', "<h4 class=\"modal-title\">".concat(this.translate('composer_remove_confirm'), "</h4>"), '</div>', '<div class="modal-body">', '</div>', '<div class="modal-footer">', "<button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">".concat(this.translate('cancel'), "</button>"), "<button type=\"button\" class=\"btn btn-primary\">".concat(this.translate('yes'), "</button>"), '</div>', '</div>', '</div>', '</div>'].join(''));
    ...
    $removeYes.on('click', () => {
      ...
      $removeDialog.modal('hide'); // Sometimes, there is a bug and the modal backdrop didn't fade out, so we have to force it
  ...
  handleContainerLoaded(event) {
    ...
    const $blockTypeSelectorLoader = $blockTypeSelector.find('.page-composer__block-type-selector__loader');
    const $blockTypeSelectorSelect = $blockTypeSelector.find('select');
    const $blockTypeSelectorButton = $blockTypeSelector.find('.page-composer__block-type-selector__confirm');
    const blockTypeSelectorUrl = $blockTypeSelectorButton.attr('href');
    applyAdmin(this.$dynamicArea); // Load the block creation form trough ajax.
    ...
    $blockTypeSelectorButton.on('click', e => {
      ...
      jQuery.ajax({
        ...
        success(resp) {
          ...
          jQuery(self).trigger(jQuery.Event('blockcreateformloaded', {
            response: resp,
    ...
    $childrenContainer.sortable({
      ...
      helper(_, element) {
        ...
        return jQuery("<div class=\"page-composer__container__child__helper\"><h4>".concat(name, "</h4></div>"));
      },

      update() {
        const newPositions = [];
        $childrenContainer.find('.page-composer__container__child').each(function containerChild(position) {
          const $child = jQuery(this);
          const parentId = $child.attr('data-parent-block-id');
          const childId = $child.attr('data-block-id'); // pending block creation has an undefined child id
  ...
  bindPagePreviewHandlers() {
    const self = this;
    this.$containerPreviews.each(function containerPreview() {
...
jQuery.fn[pluginName] = function plugin(options) {
  return this.each(function plugins() {
    if (!jQuery.data(this, "plugin_".concat(pluginName))) {
      const treeView = new TreeView(this, options);
      treeView.init();
      jQuery.data(this, "plugin_".concat(pluginName), treeView);
    }
  });
};
...
/* harmony import */ var jquery_ui_ui_widgets_droppable__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! jquery-ui/ui/widgets/droppable */ "./node_modules/jquery-ui/ui/widgets/droppable.js");
...
/* harmony import */ var _composer__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./composer */ "./assets/js/composer.js");
/* harmony import */ var _composer__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_composer__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _treeview__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./treeview */ "./assets/js/treeview.js");
/* harmony import */ var _treeview__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_treeview__WEBPACK_IMPORTED_MODULE_3__);
/*!
 * This file is part of the Sonata Project package.
 *
 * (c) Thomas Rabaix <thomas.rabaix@sonata-project.org>
